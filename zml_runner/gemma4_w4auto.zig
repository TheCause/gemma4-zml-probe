// Runner décode complet E2B-W4 (chantier W4, plan docs/superpowers/plans/2026-07-24-w4-j1-brique-e2b.md,
// spec docs/superpowers/specs/2026-07-18-w4-poids-4bit-12b-design.md) — clone ciblé de
// gemma4_gen_auto.zig (Task 7 J1) : SEULS Model/W4Step/init changent, tout le reste (RoPE host,
// chat template, tokenizer, boucle, garde VRAM 20 GiB, garde CudaRequired, flags) est à l'IDENTIQUE.
// Le forward compilé = `W4Step.forward` : assemble PAR VALEUR un Model moteur dont les poids
// linéaires sont des sous-graphes dequantW4 (w4.zig), puis délègue à `Model.forwardStep` INCHANGÉ
// (engine.zig:661) + topK — le moteur engine.zig est INTACT (0 octet).
// Modèle : `w4.W4Model` (35 W4LayerW champs communs + 15 W4KV producers 0-14 — les 20 readers
// YOCO n'ont PAS de k/v dans le checkpoint QAT, cf w4.zig toLayerW placeholder inerte).
//
// CLI : gemma4_w4auto <model.safetensors (w4a16-ct, weights_w4)> <tokenizer.json> --prompt "..."
...
// Les modes --selftest-gather/--selftest-inputs restent tels quels (clés absolues EMB_KEY/EPTL_KEY
// sur la root view, toujours présentes en bf16 dans weights_w4).
...
const usage =
    "Usage: gemma4_w4auto <model.safetensors (checkpoint w4a16-ct, weights_w4)> <tokenizer.json> --prompt \"...\" " ++
...
// device-résident dans `Model` (lm_head tied, engine.zig:487/507) : le gather du `W4Step`
...
// SG (spec [it.4]) : mini-graphe gather-only — mêmes primitives que `W4Step` (gather + GatherOpts
// `.{}` OBLIGATOIRE, cf W4Step plus bas), sans forwardStep/topK (pas besoin du modèle complet).
const SgFwd = struct {
...
// ============================================================================================
// L3 — boucle autonome prefill-par-decode (W4Step = gather + forwardStep + topK IN-GRAPH,
// buffers device per-step) : gates G1/G2/G2b (spec docs/L3_INGRAPH_DESIGN.md).
// ============================================================================================
//
// Compile UNE FOIS le mono-graphe `W4Step.forward`, qui compose : gather (`model.embed_tokens` +
...
//     lus par forwardStep (le vrai token vient désormais du gather `W4Step`, pas de `p`) —
...
//
// Risque nommé (spec §5) : 1er compile GPU du mono `W4Step.forward` (gather+forwardStep+topK) —
...
// Top5 : idx/val remplis DEPUIS LE DEVICE (topK in-graph, `W4Step` plus bas) — plus de scan host
...
// W4 : compose gather in-graph + dequant W4 in-graph + forwardStep (engine INTACT) + topK.
// top1 du topK == argmax (tri descendant `sort`, cf tensor.zig:3096) ; top5 = diagnostic
...
const W4Step = struct {
    pub fn forward(m4: w4.W4Model, tabs: Tabs, tok: zml.Tensor, p: PackedLong, cache: engine.Cache, ctrl: engine.Ctrl) struct { zml.Tensor, zml.Tensor, zml.Tensor, zml.Tensor, zml.Tensor, zml.Tensor } {
        ...
        // traçage : le tableau vit le temps du trace. kv=null pour les readers YOCO (placeholder
        // inerte, cf w4.zig toLayerW).
...
pub fn main(init: std.process.Init) !void {
    ...
    // W4 : même `base` — le checkpoint w4a16-ct garde le préfixe `model.language_model.`.
    const model: w4.W4Model = try .init(arena.allocator(), base);
    ...
    // `{ .s }` shape `[1]`, puis dans W4Step.forward : `.gather(.{ .voc = tok }).reshape(.{ 1, 1, D }).withTags(.{ .b, .s, .d })` (reshape layout-preserving + re-tag, piège ZML #1 connu) — idem `el` avec LF.
    ...
    // === Step 5.1 (suite) : compile W4Step.forward (risque nommé — cf tête de section) ===
    log.info("Compiling W4Step.forward (gather+dequant W4+forwardStep+topK, mono-graphe 35 couches) ...", .{});
    const t_compile: std.Io.Timestamp = .now(io, .awake);
    var exe = try platform.compileFn(allocator, io, W4Step.forward, .{ model, tabs, tok_sym, packed_sym, cache_sym, ctrl_sym }, .{ .shardings = &.{sharding} });
    defer exe.deinit();
    ...
    while (true) : (step += 1) {
        ...
        // + forwardStep + topK composés IN-GRAPH par W4Step (plus de Buffer.fromBytes embeds/
