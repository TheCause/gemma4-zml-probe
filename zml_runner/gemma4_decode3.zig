// P5.7.7 decode-3 — MOTEUR decode e2e Gemma-4-E2B-it (35 couches, 1 token) -> logits == HF.
//
// Le test décisif du decode : un pas de génération complet, en mode incrémental, à travers les 35
// couches. Adapté de gemma4_prefill.zig (dispatch sliding/full + runLayer + PLE frontend + YOCO) en mode
// S=1 avec caches : chaque producer (0-14) récupère son cache prefill (empaqueté en 2 tenseurs multi-slots
// sliding/full, fournis par l'oracle), append le K/V du nouveau token (scatterSlices à pos p), et attend
// sur le cache grandi ; les writers 13/14 publient leur cache grandi pour les readers 15-34 (YOCO).
// -> final norm -> last_hidden -> lm_head tied + softcap -> logits. Compare last_hidden (scan) + argmax==HF.
//
// CLI : gemma4_decode3 <model.safetensors> <p5_7_7_decode3.safetensors>
...
/// Forward decode d'UNE couche i (S=1). Producer: compute K/V du token p, scatter dans son cache prefill
/// (empaqueté), attend sur le cache grandi ; publie aux writers 13/14. Reader: réutilise shared (cache
/// grandi du writer). Puis MLP + bloc PLE + layer_scalar (identiques au prefill).
...
const Engine = struct {
    ...
    /// Forward decode e2e : embed -> PLE -> 35 couches (caches) -> final norm -> lm_head + softcap.
...
pub fn main(init: std.process.Init) !void {
    ...
    if (process_args.len < 3) {
        log.err("Usage: gemma4_decode3 <model.safetensors> <p5_7_7_decode3.safetensors>", .{});
        return error.MissingArgument;
    }
    const ckpt = process_args[1];
    const fixture = process_args[2];
    log.info("P5.7.7 decode-3 — moteur decode e2e (35 couches, 1 token)", .{});
    ...
    log.info("Materializing weights (35 couches + frontend + lm_head)...", .{});
    ...
    log.info("Compiling decode-3 forward...", .{});
    ...
    log.info("Comparaison e2e vs oracle :", .{});
    const hidden_ok = try scanHidden(allocator, io, &r_hidden, &orc_buf.last_hidden);

    // Magnitudes des logits (pas seulement l'argmax) — cf P5.7.6. Logits softcappés [-30,30] ; dérivés du
    // last_hidden via lm_head (matmul amplificateur) -> tol plus large (1e-1), critères liants = hidden+argmax.
    ...
    log.info("  argmax (token suivant) : ZML={d} HF={d} -> {s}", .{ zml_argmax, hf_argmax, if (argmax_ok) "PASS" else "FAIL" });

    if (hidden_ok and logits_ok and argmax_ok) {
        log.info("P5.7.7 decode-3 PASS — moteur decode 35 couches : last_hidden bit-near + logits + token suivant == HF", .{});
    } else {
        log.err("P5.7.7 decode-3 : divergence (hidden_ok={} logits_ok={} argmax_ok={})", .{ hidden_ok, logits_ok, argmax_ok });
