// Q5 (volet B) — BOUCLE DE GÉNÉRATION decode ZML AVEC V-QUANT (MSE V-only) -> séquence == HF-V-quant.
//
// COPIE de gemma4_decode4.zig (boucle génération decode, cache threadé, prouvée P5.7.8) + insertion du
// quantizer V (MSE V-only) au point v_norm dans la branche producer — EXACTEMENT comme gemma4_decode_vq.zig
// (Q4) le fait. La plomberie cache/boucle de decode4 n'est PAS réécrite : seul l'insert quantizeV est ajouté,
// plus le chargement des constantes codebook_256/512 + hadamard_256/512.
//
// V est 4D [.b,.s,.nh,.hd] mais en decode 1-step .b=.s=.nh=1 -> reshape à [.k=1,.hd] (axes attendus par
// quantizeV), quantifier, reshape back. Le couple {codebook,hadamard} est sélectionné par le flag full.
// K reste fp32 (V-only).
//
// Validation teacher-forcing : on feed les tokens HF-V-quant pré-gatherés (embeds dans la fixture) et on
// vérifie que l'argmax ZML de chaque step == le token suivant de HF-V-quant (séquence == HF-V-quant greedy).
//
// CLI : gemma4_gen_vq <model.safetensors> <decode_vq_gen.safetensors>
...
// MSE V-only quantizer chain — REPRISE bit-exacte de gemma4_vquant.zig (Q3) / gemma4_decode_vq.zig (Q4).
...
// Entrées par-step empaquetées + constantes V-quant (constantes sur la boucle).
const Packed = struct {
...
// Séquence attendue (HF-V-quant), lue côté host.
const ExpW = struct {
    e: zml.Tensor,
...
/// Forward decode d'UNE couche i en mode génération (cache threadé), V QUANTIFIÉ au point v_norm.
/// {cb256,Pi256,cb512,Pi512} passés en argument (constants sur la boucle).
fn runLayerGen(layer: LayerW, comptime i: usize, hidden: zml.Tensor, ple_i: zml.Tensor, cos: zml.Tensor, sin: zml.Tensor, mask: zml.Tensor, pos_s: zml.Tensor, pos_u: zml.Tensor, cb256: zml.Tensor, Pi256: zml.Tensor, cb512: zml.Tensor, Pi512: zml.Tensor, cache: *Cache) zml.Tensor {
    const full = isFull(i);
    ...
    if (reader) {
        ...
        // === Q5 : V-QUANT MSE V-only au point cache (entre v_norm et transpose) — même insert qu'en Q4 ===
        const cb = if (full) cb512 else cb256;
        const Pi = if (full) Pi512 else Pi256;
...
const Engine = struct {
    ...
    /// Un pas de génération V-quant : sélectionne le step, embed+PLE, 35 couches (cache threadé, V quantifié)
    /// -> logits + cache grandi. Retour : {logits {b,s,voc}, sl_k, sl_v, fl_k, fl_v}.
    pub fn forward(self: Engine, p: Packed, cache_in: Cache, ctrl: Ctrl) struct { zml.Tensor, zml.Tensor, zml.Tensor, zml.Tensor, zml.Tensor } {
        ...
        inline for (0..NUM_LAYERS) |i| {
            ...
            hidden = runLayerGen(self.layers[i], i, hidden, ple_i, cos, sin, mask, pos_s, pos_u, p.codebook_256, p.hadamard_256, p.codebook_512, p.hadamard_512, &cache);
        }
...
pub fn main(init: std.process.Init) !void {
    ...
    if (process_args.len < 3) {
        log.err("Usage: gemma4_gen_vq <model.safetensors> <decode_vq_gen.safetensors>", .{});
        return error.MissingArgument;
    }
    const ckpt = process_args[1];
    const fixture = process_args[2];
    log.info("Q5 — boucle génération decode V-QUANT ({d} tokens, MSE V-only)", .{NUM_STEPS});
    ...
    log.info("Materializing weights + packed inputs + caches + constantes V-quant...", .{});
    ...
    log.info("Compiling gen-vq step...", .{});
    ...
    while (step_idx < NUM_STEPS) : (step_idx += 1) {
        ...
        log.info("  step {d} (pos {d}) : argmax ZML-V-quant={d} HF-V-quant={d} -> {s}", .{ step_idx, 4 + step_idx, tok, exp, if (ok) "PASS" else "FAIL" });
    ...
    log.info("Portage V-quant : {d}/{d} tokens argmax-match (ZML-V-quant == HF-V-quant)", .{ n_match, NUM_STEPS });
    if (all_pass) {
        log.info("Q5 gen-vq PASS — le moteur ZML-V-quant génère {d} tokens, séquence == HF-V-quant greedy", .{NUM_STEPS});
    } else {
        log.err("Q5 gen-vq : divergence de séquence vs HF-V-quant ({d}/{d} match)", .{ n_match, NUM_STEPS });
