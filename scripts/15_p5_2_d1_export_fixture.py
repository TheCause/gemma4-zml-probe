"""P5.2.D.1 — Export safetensors fixture pour la sous-gate ZML k_proj.

Lit `fixtures/p5_2_d0_kv_oracle_layer13.pt` (artefact figé par D.0) et
extrait un sous-ensemble minimal pour D.1 :

    hidden_input     [B,S,H]       = [1, 4, 1536]
    k_proj_weight    [n_kv*hd, H]  = [256, 1536]
    k_after_proj     [B,S,kv]      = [1, 4, 256]   (oracle PyTorch BLAS)

Sortie : `fixtures/p5_2_d1_k_proj_layer13.safetensors` + manifest JSON.

Ne touche pas D.0. Pas de chargement modèle. Pas de RoPE. Pas de k_norm.
Reduit le footprint pour le runner ZML (3 tenseurs au lieu de 16).
...
OUT_FIXTURE = ROOT / "fixtures" / "p5_2_d1_k_proj_layer13.safetensors"
OUT_MANIFEST = ROOT / "fixtures" / "p5_2_d1_k_proj_layer13_manifest.json"

LAYER_IDX = 13
LAYER_TYPE = "sliding_attention"
EXPECTED_HIDDEN_SHAPE = (1, 4, 1536)
EXPECTED_K_PROJ_W_SHAPE = (256, 1536)
EXPECTED_K_AFTER_PROJ_SHAPE = (1, 4, 256)
...
def main() -> None:
    ...
    k_after_proj = blob["k_after_proj"]
    ...
    assert tuple(k_after_proj.shape) == EXPECTED_K_AFTER_PROJ_SHAPE, (
        f"k_after_proj shape {tuple(k_after_proj.shape)} "
        f"!= {EXPECTED_K_AFTER_PROJ_SHAPE}"
    )
    for name, t in [
    ...
    # The Zig runner reads the oracle tensor at runtime instead of hardcoding.
    print("Fixed points (k_after_proj, head 0 implicit since n_kv=1):")
    for s in (0, 1, 3):
        block = k_after_proj[0, s, :8].tolist()
        block_str = ", ".join(f"{v:.10f}" for v in block)
        print(f"  k_after_proj[0, {s}, :8] = [{block_str}]")
    print()
    ...
    tensors = {
        ...
        "k_after_proj": k_after_proj.contiguous(),
    }
    ...
    manifest = {
        "source": "P5.2.D.1 ZML k_proj-only fixture (slim from D.0)",
        ...
        "expected_zml_residual_hint": "~1e-5 (matmul PJRT-CPU Eigen-like vs PyTorch BLAS, cf C.1)",
        "interdits_p5_2_d1": [
            "v_proj",
            "k_norm",
            "RoPE",
    ...
    print()
    print("P5.2.D.1 fixture export PASS.")
