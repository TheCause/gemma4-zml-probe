"""W4 W0b/W4g — oracle de génération sur checkpoint arbitraire (fork du 49). Ajouts : --weights
(chemin du safetensors), --device {cpu,auto} défaut cpu (déterminisme, spec §3.5). Utilisé pour
l'oracle du décode E2B-W4 (fixture w4_gen48).

Dérivé de 46_gen_long_oracle.py (via 49_gen_custom_oracle.py). Différences vs 46 : le prompt vient
de --prompt (langage naturel, encodé via le CHAT TEMPLATE Gemma au lieu d'INPUT_IDS hardcodés),
SEQ_LEN = longueur du prompt tokenisé (variable), N_DECODE = --n-tokens (défaut 200, pas 1020).
Produit le MÊME format de fixture que 46 (embeds, embptls, cos/sin full, positions,
masks_sliding/full .k=L_MAX, caches prefill paddés, expected/fed) → reproductible tel quel par
`gemma4_gen_long_gpu` (L_MAX=1024). Détokenise ensuite avec scripts/48_detokenize.py.

Le moteur ZML reste un BANC op-par-op : HF (ici) est l'oracle de référence ; ZML doit reproduire
`expected` au token près. La chaîne complète testée : prompt texte → (HF tokenise+génère) →
fixture → (ZML reproduit) → (48 détokenise + valide round-trip) → texte.

CLI : python3 scripts/56_w4_gen_oracle.py --prompt "..." [--weights .../model.safetensors]
      [--device cpu|auto] [--n-tokens 200] [--out gen_custom.safetensors]
Prérequis : venv gemma4-probe, weights + tokenizer en cache. --device cpu par défaut (déterminisme,
recommandé pour le gate W0) ; --device auto bascule sur cuda si disponible (sinon cpu).
"""
...
def main() -> None:
    ...
    ap.add_argument("--weights", default=str(ROOT / "weights" / "model.safetensors"),
                    help="chemin du safetensors à charger (W0b : weights_w4dq/model.safetensors)")
    ap.add_argument("--device", choices=["cpu", "auto"], default="cpu",
                    help="cpu (déterminisme, défaut, spec §3.5) ou auto (cuda si dispo sinon cpu)")
    ...
    DEVICE = "cpu" if args.device == "cpu" else ("cuda" if torch.cuda.is_available() else "cpu")
