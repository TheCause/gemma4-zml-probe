...
> **✅ Statut : Task 1 (E1) + Task 2 (E2) DONE — gates `engine-e1-noregression-pass` + `engine-e2-brick-pass`.**

> **📦 État publication** — Sur `main` (public) : socle `engine.zig` corrigé + gate E1 uniquement. La brique
...
- Compute `ssh user@gpu-host`. Workspace ZML `/data/rqz_workspace/zml/examples/rqz/`. Build : `ssh user@gpu-host 'cd /data/rqz_workspace/zml && ./bazel.sh build //examples/rqz:<cible> 2>&1 | tail -3'`. Run : idem + `./bazel-bin/examples/rqz/<cible> <fixture>`. Déploiement `scp <f> user@gpu-host:/data/rqz_workspace/zml/examples/rqz/`. **Écrire que sur /data.**
