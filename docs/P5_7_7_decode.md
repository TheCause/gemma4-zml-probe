...
à pos p ; `attention_k_eq_v=False` confirmé → V séparé ; 6/6 PASS, tag `p5.7.7-decode2-full-pass`) →
**decode-3** (35 couches : 2 caches multi-slots, dispatch, → norm → **logits → argmax == HF**, test e2e
décisif) → **P5.7.8** (boucle génération N tokens). **Les 2 types de couche en decode sont prouvés.**
