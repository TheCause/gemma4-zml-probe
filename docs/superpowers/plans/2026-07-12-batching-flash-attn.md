...
- Baseline: worktree du commit courant AVANT modification (méthode G2.3.0 gold)

- [ ] **Step 1: Produire la baseline HLO AVANT modification**

Sur la 3090, binaire gen_auto ACTUEL :
```bash
git -C /data/gemma4-zml-probe worktree add /tmp/probe_baseline HEAD   # ou rsync du HEAD M1
# déployer zml_runner/ du HEAD inchangé, puis :
...
Dans `engine.zig`, remplacer la consommation des constantes `B`/`S` par des dims dérivées du
tensor en scope (couper la chaîne — le receiver n'est pas référençable dans ses arguments) :

```zig
// Site 1 (q, ligne ~395) — h0 {b,s,d} est en scope :
const q = dotPrec(prec.q_proj, h0, layer.q_proj, .d)
    .reshape(.{ h0.dim(.b), h0.dim(.s), NH, hd })
    .withTags(.{ .b, .s, .nh, .hd });
// Sites 2 et 3 (k ~412, v ~417) : idem avec KVH.
...
- Modify: `zml_runner/BUILD.bazel` (nouvelle cible, pattern `gemma4_gen_auto` :373-381)
- Create: `docs/BATCHING_RESULTS.md` (squelette des sections T0→B4, S1→S3)

- [ ] **Step 1: Cible Bazel**

Dans `BUILD.bazel`, copier le bloc zig_binary de `gemma4_gen_auto` : `name = "gemma4_bbatch"`,
...
- CLI (`gen_auto.zig:79-150` comme modèle) : positionnels `<model> <tokenizer>`, flags
  `--prompts <fichier>` (un prompt/ligne), `--oracles <f1,f2,...>`, `--replicate N`,
  `--max-tokens N`, `--force-vram`, `--allow-cpu`, `--no-prealloc`, `--ids-only`,
  `--selftest-batch <fixture>` (stub pour Task 3) ;
...
- [ ] **Step 3: Vérifier le mode host-only sur M1 (pas de GPU requis)**
...
./bazel-bin/examples/rqz/gemma4_bbatch <model> <tok> --selftest-batch fixtures/<sg_fixture>.safetensors --prompt "x"
```
...
- Create: `fixtures/bench_prompts_b4.txt` (jeu versionné, 4 prompts distincts même longueur)
- Modify: `zml_runner/gemma4_bbatch.zig` (mode `--oracles`)
...
(ajuster le texte jusqu'à égalité). Committer `fixtures/bench_prompts_b4.txt`.
...
--out /data/fixtures/oracle_lane<i>.safetensors` (une par prompt, GPU requis, HF offline).
Vérifier dans chaque manifest sidecar : `seq_len` identique partout.
...
`positions[0]==ids.len` par fixture, diagnostic top-5 au premier mismatch d'une lane
(step fautif, marge top1−top2 — procédure d'échec spec §4).

- [ ] **Step 4: Gate B2 — B=2 puis B=4**

```bash
... gemma4_bbatch <model> <tok> --oracles /data/fixtures/oracle_lane0.safetensors,/data/fixtures/oracle_lane1.safetensors
... gemma4_bbatch <model> <tok> --oracles <les 4>
```
...
- [ ] **Step 1: Choisir le prompt** (EOT dans la fenêtre des 48 steps si possible — sinon
  documenter la clause EOT vacuous, spec §6 B3) et lancer B=4 même prompt ×4
  (`--prompts` avec 4 lignes identiques, ou `--replicate 4` sur 1 prompt).

- [ ] **Step 2: Vérifier** : ids u32 identiques sur les 4 lanes à chaque step actif +
  même step d'EOT. Le runner fait cette comparaison en mode `--replicate` automatiquement
  (log `B3 : lanes identiques 48/48 steps`).

- [ ] **Step 3: Consigner + commit + tag** (`gate/batch-b3-pass`).
...
- [ ] **Step 2: Écrire `scripts/61_batch_sweep.sh`** : build unique + sha256 consigné/vérifié
  avant chaque run, boucle sur B, runs appariés gen_auto/bbatch à B=1, échantillonneur VRAM
  (boucle `nvidia-smi --query-compute-apps` pendant le run long), écriture du manifest
...
const ctx_out = if (comptime cfg.attn == .sdpa) blk: {
    const un = zml.Tensor.scalar(1.0, qs.dtype());          // scaling Gemma = 1.0, JAMAIS 1/√hd
...
- [ ] **Step 1: Câbler un flag de build/CLI** pour instancier `Model` avec `.attn = .sdpa`
  dans bbatch (option comptime via second binaire `gemma4_bbatch_sdpa` dans BUILD.bazel —
  2 cibles, même source, `zig_binary` avec `-D` ou struct différent ; choisir le mécanisme le
  plus court en @typeName).
