...
|---:|---:|---:|
| 1280 | 13 MiB | 0,88 GiB |
| 4096 | 134 MiB | 2,81 GiB |
| 8192 | 537 MiB | 5,62 GiB |
| 16384 | 2,15 GiB | 11,2 GiB |
...
`EngineCfg` (engine.zig) gagne deux champs à défaut neutre :

```zig
ingraph_masks: bool = false, // two_masks=true requis ; génère les masques depuis positions[step]
sliding_window: i64 = 0,     // requis si ingraph_masks (12B : 1024) ; 0 = interdit si actif
```

Garde comptime : `ingraph_masks and !two_masks` → `@compileError` ; `ingraph_masks and
sliding_window <= 0` → `@compileError`. (Le chemin mono-masque E2B n'est pas couvert : YAGNI.)
...
- `.ingraph` = `.tables` **sans** `masks_sliding`/`masks_full` (les 5 autres champs
  inchangés : embeds, embptls, cos_full, sin_full, positions).
...
iota_k   = Tensor.iota({k = KMAX}, .k)            // i32 (zml tensor.zig:2014)
full     = select(iota_k <= pos, 0.0, MASK_MIN)   // f32
sliding  = select((iota_k <= pos) and (iota_k >= pos − (SW−1)), 0.0, MASK_MIN)
```
...
- `HostTables` n'alloue/ne remplit plus `masks_sliding`/`masks_full` en mode ingraph
  (gain host : 134 MiB à 4k, 537 MiB à 8k ; gain device idem ; le `dynamicSlice` masque
  par step disparaît aussi).
...
- Le selftest inputs actuel compare les tables host aux fixtures — la partie masques n'a
  plus d'objet en ingraph. Remplacement : **selftest croisé host** — un mini-forward de
  sonde n'est PAS nécessaire ; on garde `maskRows` host comme référence et on prouve
  l'équivalence par les sorties (M1/M2) + le contre-test de fenêtre existant
  `--window-vacuity` (U9-ii) qui prouve **in-process** que la fenêtre 1024 mord encore
  (anti « masque devenu vide » — leçon vacuité de l'antécédent).
- M3 exécute `--window-vacuity` à 8k : la fenêtre doit mordre à q ≥ 1024 exactement
  comme à 4k.
...
| **M1** équivalence 1280 | `gemma4_g12auto` ingraph vs binaire témoin chemin `.tables` | **48/48 ids free-run identiques** (critère PR #13 défaut) | 3090 |
| **M2** équivalence 4096 | `gemma4_g12a4k` ingraph vs témoin `.tables` | **300/300 préfixe identique** (critère PR #13 variante) | 3090 |
