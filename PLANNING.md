...
Tag courant : `p5.2-f-oproj-zml-pass`. **P5.2.E COMPLET + P5.2.F (o_proj) PASS.** Reste : `post_attention_layernorm`+résiduel (→ sous-couche attention complète), MLP (gate/up/down+activation), layer 14 full attention (p-RoPE proportional, patch zml.nn.rope requis), vrai sliding au compute (S≥512).
