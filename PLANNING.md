...
Tag courant : `p5.2-h-mlp-zml-pass`. **COUCHE DÉCODEUR SLIDING COMPLÈTE** (attention E/F/G + MLP H, tous maillons validés). **MODE AUTONOME** (1 juin) : Régis a demandé d'aller jusqu'au bout du projet sans l'attendre, revue cross-LLM ensuite. Plan = `docs/ROADMAP_to_full_forward.md`. Ordre : P5.3 (assemblage couche sliding e2e) → P5.4 (embedding+scale) → P5.5 (final norm+lm_head+softcap) → P5.6 (layer 14 full attn, RoPE manuelle) → P5.7 (multi-couches end-to-end).
